...
attributes(win_raster) <- data.frame(id = sapply(feb, function(x) paste(x$AnimalID[1])),
                                      interval_start_date = sapply(feb, function(x) paste(x$start[1])),
                                      year = paste0(sapply(feb, function(x) paste(x$year[1])))
)
...
attributes(sum_raster) <- data.frame(id = sapply(july, function(x) paste(x$AnimalID[1])),
...
attributes(env_list) <- data.frame(id = sub("\\.tif", "", basename(names(env_list))),
                             year = trimws(basename(names(env_list)), whitespace = "_.*"))

sum_rst <- env_raster_stack(env_list, sum_raster, attribute = "year")
win_rst <- env_raster_stack(env_list, win_raster, attribute = "year")
