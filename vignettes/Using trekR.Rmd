...
### Create intervals
We created 10-day intervals for unicorns in the January and July. We first created a `new` column using the `floor_Date()` function to obtain the 10-day extents within the months. We included the 31st day (if a month had 31 days) in the third 10-day interval to prevent the creation of a 4th 10-day interval with only a single day in it.   
...
jan <- unicorns %>%
  ...
  # previous interval. 
  group_by(ID) %>% # Included if more than a single individual is included
...
```{r}
btwavg_geo / wavg_geo_sum
btwavg_geo / wavg_geo_win
...
We first import the environmental co variate file, then we reproject the extent from latitude and longitude to UTMs to match those of the home range rasters.
```{r} 
  #Load raster
  UtahDEM <- raster::raster("Utah_DEM.tif")
