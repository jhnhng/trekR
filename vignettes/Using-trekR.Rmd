---
title: "Using the functions in `trekR`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Using the functions in `trekR`}
...
```
# Geographical Space
btwemd_geo <- between_emd(seasons[[1]], seasons[[2]])
...
```
# Geographical Space
withsum_geo <- within_emd(seasons[[1]])
withwin_geo <- within_emd(seasons[[2]])
...
We calculated the EM-speeds between seasons (e.g., first intervals between `jan` and `july`), and within seasons (e.g., first and second intervals between `jan` and `july`) using the `emspeeds()` in our `trekR` package. The example below demonstrates the calculation for the EM-speeds for geographical space, but can easily be replaced with `btwemd_env`, `withsum_env` and `withwin_env`.
```
...
wavg_geo_sum <- lapply(wesp_sum, function(x) mean(unique(x[x!= 0])))
wavg_geo_win <- lapply(wesp_win, function(x) mean(unique(x[x!= 0])))
```

The ratio between the two averages are the migratory predictors and are limitless. 

```
sum <- Map("/", btwavg_geo, wavg_geo_sum)
win <- Map("/", btwavg_geo, wavg_geo_win)
...
out <- transform(read.table(text = unlist(unique(names(win_raster))), header = FALSE, 
                 ...
                 sum_geo = unlist(sum), win_geo = unlist(win))
