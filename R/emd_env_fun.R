...
emd_env <- function(rs1, rs2, nbins = 50) {
  ...
  ud1 <- raster::values(rs1)[, 1]
  ud1[is.na(ud1)] <- 0
  ud2 <- raster::values(rs2)[, 1]
  ud2[is.na(ud2)] <- 0

  # Get environmental covariate values
  env1 <- raster::values(rs1)[, 2]
  env2 <- raster::values(rs2)[, 2]
  env_all <- c(env1, env2)
  ...
  # Summarize by bin
  agg1 <- stats::aggregate(env_df1$ud, list(env_df1$bin), sum, na.rm = TRUE)
  names(agg1) <- c("bin", "ud")
  agg2 <- stats::aggregate(env_df2$ud, list(env_df2$bin), sum, na.rm = TRUE)
  names(agg2) <- c("bin", "ud")
